...
func (decoder *addressDecoder) PrivateKeyToWIF(priv []byte, isTestnet bool) (string, error) {
	...
	cfg := gxtglobal_addrdec.GXT_mainnetPrivateWIFCompressed
	if decoder.wm.Config.IsTestNet {
		cfg = gxtglobal_addrdec.GXT_testnetPrivateWIFCompressed
	}

	wif, _ := gxtglobal_addrdec.Default.AddressEncode(priv, cfg)
...
func (decoder *addressDecoder) PublicKeyToAddress(pub []byte, isTestnet bool) (string, error) {
	gxtglobal_addrdec.Default.IsTestNet = decoder.wm.Config.IsTestNet
	address, err := gxtglobal_addrdec.Default.AddressEncode(pub)
	if err != nil {
		return "", err
	}
...
func (decoder *addressDecoder) RedeemScriptToAddress(pubs [][]byte, required uint64, isTestnet bool) (string, error) {
	...
	cfg := gxtglobal_addrdec.GXT_mainnetAddressP2SH
	if decoder.wm.Config.IsTestNet {
		cfg = gxtglobal_addrdec.GXT_testnetAddressP2SH
	}
	...
	address, _ := gxtglobal_addrdec.Default.AddressEncode(pkHash, cfg)
...
func (decoder *addressDecoder) WIFToPrivateKey(wif string, isTestnet bool) ([]byte, error) {
	...
	priv, err := gxtglobal_addrdec.Default.AddressDecode(wif, cfg)
	if err != nil {
		return nil, err
	}
