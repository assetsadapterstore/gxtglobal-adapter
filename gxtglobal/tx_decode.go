...
func (decoder *TransactionDecoder) SignBTCRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	if keySignatures != nil {
		for _, keySignature := range keySignatures {
			...
			txHash := gxtTransaction.TxHash{
				Hash: keySignature.Message,
				Normal: &gxtTransaction.NormalTx{
					Address: keySignature.Address.Address,
					SigType: gxtTransaction.SigHashAll,
				},
			}
			...
			//签名交易
			/////////交易单哈希签名
			sigPub, err := gxtTransaction.SignRawTransactionHash(txHash.GetTxHashHex(), keyBytes)
			if err != nil {
...
func (decoder *TransactionDecoder) VerifyBTCRawTransaction(wrapper openwallet.WalletDAI, rawTx *openwallet.RawTransaction) error {
	...
	var (
		txUnlocks  = make([]gxtTransaction.TxUnlock, 0)
		emptyTrans = rawTx.RawHex
		//sigPub     = make([]gxtTransaction.SignaturePubkey, 0)
		transHash     = make([]gxtTransaction.TxHash, 0)
		addressPrefix gxtTransaction.AddressPrefix
	)
	...
	for accountID, keySignatures := range rawTx.Signatures {
		...
		for _, keySignature := range keySignatures {
			...
			signaturePubkey := gxtTransaction.SignaturePubkey{
				Signature: signature,
				Pubkey:    pubkey,
			}
	...
	trx, err := gxtTransaction.DecodeRawTransaction(txBytes, decoder.wm.Config.SupportSegWit)
	if err != nil {
	...
	for _, vin := range trx.Vins {
		...
		txUnlock := gxtTransaction.TxUnlock{
	...
	signedTrans, err := gxtTransaction.InsertSignatureIntoEmptyTransaction(emptyTrans, transHash, txUnlocks, decoder.wm.Config.SupportSegWit)
	if err != nil {
	...
	pass := gxtTransaction.VerifyRawTransaction(signedTrans, txUnlocks, decoder.wm.Config.SupportSegWit, addressPrefix)
	if pass {
...
func (decoder *TransactionDecoder) createBTCRawTransaction(
	...
	var (
		err              error
		vins             = make([]gxtTransaction.Vin, 0)
		vouts            = make([]gxtTransaction.Vout, 0)
		txUnlocks        = make([]gxtTransaction.TxUnlock, 0)
		totalSend        = decimal.New(0, 0)
		destinations     = make([]string, 0)
		accountTotalSent = decimal.Zero
	...
	for _, utxo := range usedUTXO {
		in := gxtTransaction.Vin{utxo.TxID, uint32(utxo.Vout)}
		vins = append(vins, in)

		txUnlock := gxtTransaction.TxUnlock{LockScript: utxo.ScriptPubKey, SigType: gxtTransaction.SigHashAll}
		txUnlocks = append(txUnlocks, txUnlock)
	...
	for to, amount := range to {
		...
		out := gxtTransaction.Vout{to, uint64(amount.IntPart())}
	...
	/////////构建空交易单
	emptyTrans, err := gxtTransaction.CreateEmptyRawTransaction(vins, vouts, lockTime, replaceable, addressPrefix)
	...
	////////构建用于签名的交易单哈希
	transHash, err := gxtTransaction.CreateRawTransactionHashForSig(emptyTrans, txUnlocks, decoder.wm.Config.SupportSegWit, addressPrefix)
	if err != nil {
...
func (decoder *TransactionDecoder) createOmniRawTransaction(
	...
	for _, utxo := range usedUTXO {
		...
		txUnlock := omniTransaction.TxUnlock{LockScript: utxo.ScriptPubKey, SigType: gxtTransaction.SigHashAll}
		txUnlocks = append(txUnlocks, txUnlock)
